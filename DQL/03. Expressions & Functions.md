...
### NULL Identification (Less Common)

| **Function** | **Description** | **Sample Code** |
| --- | --- | --- |
| ISNULL(expr) *(Alias for IFNULL() in MySQL)* | Returns 1 if expr is NULL, else 0 | SELECT ISNULL(NULL);  -- Returns 1 |

⚠️ ISNULL() is non-standard in MySQL and behaves differently in SQL Server.
